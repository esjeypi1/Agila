        ...
        <section class="hero-section"> <!-- Main Menu Container (START) -->
            ...
            <div class="left-menu"> <!-- Side Menu (START) -->
                ...
                <div class="side-btns">
                    ...
                    <a class="as-btn" href="ps.php"><span class="material-symbols-outlined">local_police</span>Stations</a>
                </div>
            ...
            <div class="right-menu"> <!-- Right Canvas (START) -->
                <div class="patterns">
                    <svg width="300%" height="120%">
                        ...
                        <text x="74%" y="90%" text-anchor="middle">Stations</text>
                    </svg>
                </div>
                        ...
                        <table id="table" class="table table-striped table-bordered table-hover" style="width:102%;">
                            <thead>
                                <tr>
                                    ...
                                    <th class="header-color">Station Number</th>
                                    <th class="header-color">Station Name</th>
                                    <th class="header-color">Latitude</th>
                                    <th class="header-color">Longitude</th>
                            ...
                            <tbody>
                                ...
                                $sql = "SELECT id_ps, num, name, lng, lat, is_hidden FROM stations";
        ...
        $(document).ready(function() {
            ...
            $('#table tbody').on('click', '.delete', function() {
                if (confirm('Are you sure you want to hide this row?')) {
                    ...
                    $.ajax({
                        url: 'delete_ps.php', // URL to the PHP script
                        type: 'POST',
                        data: {
                            id_ps: rowId
                        },
                            ...
                            if (response.status == 'success') {
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        ...
                                        item_type: 'station',
                                        item_id: rowId,
                                        details: `Admin hidden the station ID: ${rowId}`
                                    },
                                    dataType: 'json',
            ...
            $('#table tbody').on('click', '.restore', function() {
                ...
                $.ajax({
                    url: 'restore_ps.php', // URL to the PHP script
                    type: 'POST',
                    data: {
                        id_ps: rowId
                    },
                        ...
                        if (response.status == 'success') {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    item_type: 'station',
                                    item_id: rowId,
                                    details: `Admin restored the station ID: ${rowId}`
                                },
                                dataType: 'json',
                                success: function(logResponse) {
                                    console.log(logResponse);
            ...
            $('.add-new').on('click', function() {
                table.row.add(['', '<input type="number" value=""/>', '<input type="text" value=""/>', '<input type="text" value=""/>', '<input type="text" value=""/>','<button class="btn btn-outline-success save-new" title="Save" data-toggle="tooltip"><i class="material-icons">&#xE161;</i></button>']).draw();
            });
            ...
            $('#table tbody').on('click', '.save-new', function() {
                ...
                // Get new values
                var newNum = $cols.eq(1).find('input').val();
                var newStation = $cols.eq(2).find('input').val();
                var newLng = $cols.eq(3).find('input').val();
                var newLat = $cols.eq(4).find('input').val();

                // AJAX request to save the new row in the database
                $.ajax({
                    url: 'upd_ps.php', // URL to the PHP script
                    type: 'POST',
                    data: {
                        num: newNum,
                        station: newStation,
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 'success') {
                            ...
                            // Update table data
                            table.row($row).data([newId, newNum, newStation, newLng, newLat, '<button class="btn btn-outline-danger delete" title="Delete" data-toggle="tooltip"><i class="material-icons">visibility_off</i></button>']).draw();
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    item_type: 'station',
                                    item_id: newId,
                                    details: `Admin added new station: ${newStation}, at: ${newLng}, ${newLat}`
                                },
                                dataType: 'json',
                                success: function(logResponse) {
                                    console.log(logResponse);
